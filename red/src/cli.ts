...
import { neonMultiNodeWorkflow } from "./workflow.ts";
...
export const usage =
  ...
  "  create    provision five machines, converge every role, run the gates\n" +
  "  delete    explicitly remove the SSH aliases, DNS, and infrastructure\n" +
  "  rehearse  rehearse quorum and storage recovery with exact witnesses\n" +
  "  describe  inspect every role container over SSH";
...
export async function run(...args: string[]): Promise<Opts> {
  ...
  if (lifecycleCommands.includes(command)) {
    return runCli(neonMultiNodeWorkflow, withFile);
  }
...
export async function exec(args: string[] = Bun.argv.slice(2)): Promise<never> {
  if (lifecycleCommands.includes(args[0] ?? "")) {
    return execCli(neonMultiNodeWorkflow, defaultArgs(args));
  }
