...
from .workflow import neon_multi_node_workflow
...
USAGE = ("Usage: blue <build|create|delete|rehearse|describe> "
         ...
         "  create    provision five machines, converge all roles, run the gates\n"
         "  delete    explicitly remove the SSH aliases, DNS, and infrastructure\n"
         "  rehearse  exercise WAL quorum and pageserver recovery\n"
         "  describe  inspect every runtime role over SSH")
...
async def run(*args):
    ...
    if command in LIFECYCLE:
        return await run_cli(neon_multi_node_workflow, args)
